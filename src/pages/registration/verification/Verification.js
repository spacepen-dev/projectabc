...
import { SaveToken, SaveUserEmail } from "../../../lib/sharedfuntions";
...
function Verification({ SignupVerification }) {
	...
	function handleOtp(elem, index) {
		...
		setOtp([
			...otp.map((input, indx) => (index === indx ? elem.target.value : input)),
		]);
		if (
		...
		(value) => {
			SignupVerification(value, (res) => {
				if (res) {
					...
					const { error, success, user_token, emailAddress } = res.data;
				...
				<main className=" container">
					<div className="h-75 w-100 d-flex align-items-center flex-column   px-5">
						<div className="w-100 otp-header">
							<h2 className="mb-3 py-2 text-center text-black fw-bold">
								Register Email Confirmation
							</h2>
						</div>
						<div className="otp-sub-header text-center fw-bold">
							...
							{otp.map((data, index) => {
								return (
									<input
										...
										type="text"
										maxLength="1"
										className="text-center otp-input rounded"
										style={{
											height: "4rem",
											width: "4rem",
											border: "1px solid rgba(98, 70, 213, 1)",
										}}
										value={data}
						...
						<div className="py-4 mt-3 w-75 ml-auto fs-6 text-center">
							...
							{state.message && !state.resendOTP && (
								<p className="text-success mt-3">{state.message}</p>
							)}
